...
vec3 & vec3::rotateBy(const quat& q)
{
	mat3 M;
	quat_2_mat(M, q);
	vec3 dst;
...
// v is normalized
// theta in radians
mat3 & mat3::set_rot(const nv_scalar& theta, const vec3& v)
{
...
mat3 & mat3::set_rot(const vec3& u, const vec3& v)
{
	nv_scalar phi;
	nv_scalar h;
	nv_scalar lambda;
...
mat4 & mat4::set_rot(const quat& q)
{
	mat3 m;
	q.ToMatrix(m);
	set_rot(m);
...
// v is normalized
// theta in radians
mat4 & mat4::set_rot(const nv_scalar& theta, const vec3& v)
{
	nv_scalar ct = nv_scalar(cos(theta));
	nv_scalar st = nv_scalar(sin(theta));
...
mat4 & mat4::set_rot(const vec3& u, const vec3& v)
{
	nv_scalar phi;
	nv_scalar h;
	nv_scalar lambda;
...
mat4 & mat4::set_rot(const mat3& M)
{
...
mat4 & mat4::set_scale(const vec3& s)
{
	a00 = s.x;
	a11 = s.y;
	a22 = s.z;
...
mat4 & mat4::as_scale(const vec3& s)
{
...
mat4 & mat4::as_scale(const nv_scalar& s)
{
	memcpy(mat_array, array16_id, sizeof(nv_scalar) * 16);
	a00 = s;
	a11 = s;
...
mat4 & mat4::set_translation(const vec3& t)
{
	a03 = t.x;
	a13 = t.y;
	a23 = t.z;
...
mat4 & mat4::as_translation(const vec3& t)
{
	memcpy(mat_array, array16_id, sizeof(nv_scalar) * 16);
	a03 = t.x;
	a13 = t.y;
...
// TL
void quat::ToEulerXYZ(vec3 & r)
{
	double a = 2.0f * (w*x + y*z);
	double b = 1.0 - 2.0f * (x*x + y*y);
	r.x = (nv_scalar)atan2(a, b);

	a = 2.0 * (w*y - z*x);
	r.y = (nv_scalar)asin(a);

	a = 2.0 * (w*z + x*y);
	b = 1.0 - 2.0 * (y*y + z*z);
	r.z = (nv_scalar)atan2(a, b);
}

quat::quat(const vec3& eulerXYZ)
{
	FromEulerXYZ(eulerXYZ);
}

void quat::FromEulerXYZ(vec3 r)
{
	r *= 0.5f;
